...
# Run tests
python3 -m django test openstax_accounts --settings=openstax_accounts.test_settings
...
- `openstax_accounts/functions.py` — Public API. Provides `get_logged_in_user_uuid(request)`, `get_logged_in_user_id(request)`, `get_token()`, `get_user_info(uid)`, and `get_user_info_by_uuid(uuid)`. All read from Django settings and `request.COOKIES`.
...
Django >= 4.2, joserfc, requests, requests-oauthlib. Python 3.9+.
